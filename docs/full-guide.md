...
个股详细报告会基于日线 `high` / `low` / `close` 计算并固定展示标准 `KDJ(9,3,3)` 的 K、D、J 数值，以及 K/D 金叉、死叉和超买超卖提示。KDJ 同时进入传统分析和 Agent 分析的技术上下文，但只作为短线辅助信息，不改变现有综合评分或操作建议。

- K、D 同时不低于 80：标记超买提示。
- K、D 同时不高于 20：标记超卖提示。
- K 上穿 D：标记金叉；K 下穿 D：标记死叉。
- 当计算窗口最高价等于最低价时，RSV 使用中性值 50，避免除零。

KDJ 在震荡市场更敏感，在强趋势中可能长期钝化，应结合均线、MACD、成交量和风险信息使用。
