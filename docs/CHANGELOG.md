...
- [新功能] 支持企业微信智能机器人 API 长连接：可在不开放公网回调的前提下接收群内查股和常用股管理命令。
- [新功能] 企业微信智能机器人新增任意股票的轻量自由问答：基于实时行情、公司公告和最近报告进行单次模型回答，无需开启完整 Agent，临时查询不会自动加入自选。
- [新功能] 企业微信常开电脑新增低噪音自选股监控，支持重大/业绩公告、异常涨跌和关键技术位跨越的去重盘中提醒。
- [新功能] 企业微信常开电脑支持 A 股交易日 14:30 独立尾盘简报，基于实时价与 MA、MACD、RSI、KDJ 生成持仓/空仓条件计划，并按交易日去重。
- [修复] GitHub Actions 日报任务统一使用 Asia/Shanghai，避免海报生成时间显示为 UTC 而与实际收取时间相差 8 小时。
- [改进] 企业微信日报改为按股票拆分的 1920px 高清卡片与大字号渲染，压缩时优先保持原始尺寸，避免超长整图在群聊预览中被过度缩小。
- [新功能] 新增无模型快速行情查询与常用股二次确认命令，避免临时查询直接扩大日报和提醒范围。
- [新功能] 个股详细报告新增标准 KDJ(9,3,3) 的 K、D、J 数值及金叉、死叉、超买超卖提示，并将确定性结果注入传统与 Agent 分析上下文；KDJ 暂不改变现有综合评分。
